...
class TestPaddleMappers(TestCase):
    ...
    def test_get_subscriber_by_payload_mixed_case_email(self):
        # Check that when subscribers are looked up using email address that the lookup is done in a case insensitive way
        Subscriber = settings.get_subscriber_model()
        test_user = Subscriber(email="user@email.com")
        test_user.save()

        payload = {"email": "user@EMAIL.com"}
        subscriber = mappers.get_subscriber_by_payload(Subscriber, payload)
        self.assertEqual(subscriber, test_user)
